package sailpoint.plugin.emphierarchyplugin.rest;
...
import sailpoint.plugin.emphierarchyplugin.server.EmpNode;
...
@SPRightsRequired(value = { "EmpHierarchyAccessRestServiceAllow" })
@Path("emphierarchyplugin")
public class EmpHierarchyResource extends AbstractPluginRestResource {
	 ...
	 * 
	 */
	public EmpHierarchyResource() throws GeneralException {
		log.info("Constructor: EmpHierarchyResource");
		try {
			Plugin plugin = PluginRegistry.get("emphierarchyplugin");
			if (plugin != null) {
				Attributes settingsAttrs = plugin.getConfigurableSettings();
				if (settingsAttrs.containsKey("attrName")) {
					attrName = (String) settingsAttrs.get("attrName");
				}
			}
		} catch (GeneralException ge) {
			throw new GeneralException("Plugin ConfigurableSettings error. check attribute value");
		}
